 ...
 */
export function heatmapsToPointsSoft(hm, inputSize = 256) {
  const dims = hm.dims;
  const data = hm.data;
  ...
  for (let i = 0; i < n; i++) {
    ...
    let sx = 0;
    let sy = 0;
    const y0 = Math.max(0, yy - rad);
    const y1 = Math.min(h - 1, yy + rad);
    const x0 = Math.max(0, xx - rad);
    const x1 = Math.min(w - 1, xx + rad);
    for (let y = y0; y <= y1; y++) {
      for (let x = x0; x <= x1; x++) {
        let v = data[off + y * w + x];
        if (v < 0) v = 0;
        sum += v;
        sx += v * x;
        sy += v * y;
