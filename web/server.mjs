#!/usr/bin/env node
/**
 * Static server for browser ONNX demo.
 * Serves ../models/shgnet + this folder.
 * Binds 0.0.0.0 — open http://127.0.0.1:8765 or http://<lan-ip>:8765
 */
...
const server = http.createServer((req, res) => {
  ...
  fs.readFile(filePath, (err, data) => {
    ...
    // COOP/COEP unlock SharedArrayBuffer for ORT multi-thread WASM (same-origin).
    res.writeHead(200, {
      "Content-Type": MIME[ext] || "application/octet-stream",
      "Cache-Control": ext === ".onnx" || ext === ".wasm" ? "public, max-age=3600" : "no-cache",
...
server.listen(PORT, HOST, () => {
  const shg = path.join(ROOT, "models/shgnet/hourglass_2stack.onnx");
  ...
  console.log(`Ear landmark ONNX web (no .pth)`);
